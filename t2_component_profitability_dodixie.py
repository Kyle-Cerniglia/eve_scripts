#!/usr/bin/env python3
"""
T2 Advanced Component profit checker for Dodixie.

- Uses ESI market data from The Forge.
- Material cost: lowest Dodixie SELL orders.
- Output price: highest Dodixie BUY orders.
- Assumes all blueprints are ME 10.
- Applies configurable cost multipliers:
    MATERIAL_COST_MULTIPLIER (e.g. 0.9 for ME10)
...
THE_SINQLAISON_REGION_ID = 10000032
DODIXIE_920_STATION_ID = 60011866  # Dodixie IX - Moon 20 - Federation Navy Assembly Plant

# Change these to taste:
MATERIAL_COST_MULTIPLIER = 0.9    # Used to account for ME10, without ceiling rounding for small batches of 1 unit
...
def get_best_order_in_jita(
    type_id: int,
    is_buy: bool,
    region_id: int = THE_SINQLAISON_REGION_ID,
    dodixie_station_id: int = DODIXIE_920_STATION_ID,
    max_pages: int = 10,
    ...
    while page <= max_pages:
        ...
        for order in orders:
            if order.get("location_id") != dodixie_station_id:
                continue
